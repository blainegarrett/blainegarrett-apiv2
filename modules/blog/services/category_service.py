...
from ..entities import CategoryEntity,  MediaEntity
...
def get_by_slug(slug: str) -> Optional[CategoryModel]:
    ...
    with client.context():
        ...
        return CategoryModel(
            resource_id=get_resource_id_from_key(e.key), name=e.name, slug=e.slug)
...
def get_categories(limit: int, cursor: str, sort: str) -> CategoryModelPagedResult:
    models: List[CategoryModel] = []
    ...
    with client.context():
        ...
        data = [c for c in results]
        for c in data:
            # logging.error(get_resource_id_from_key(c.key))
            models.append(CategoryModel(
                resource_id=get_resource_id_from_key(c.key), name=c.name, slug=c.slug))
