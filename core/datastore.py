...
def get_resource_id_from_key(key):
    ...
    for pair in pairs:
        ...
        if isinstance(key_or_id, int):
            key_or_id = unicode(str(INTPREFIX) + unicode(key_or_id))

        pair_strings.append(kind + str(SEPARATOR) + key_or_id)

    buff = SEPARATOR.join(pair_strings).encode()
...
def get_key_from_resource_id(resource_id):
    ...
    try:
        ...
        raise InvalidIdException(
            argResourceId + ' does not appear to be a valid resource_id')

    key_pairs = []
    bits = decoded.split(SEPARATOR.encode())

    for bit in bits:
        if (chr(bit[0]) == INTPREFIX):
            bit = int(bit[1:])
        else:
            bit = bit.decode()
